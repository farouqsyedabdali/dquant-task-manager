        ...
        {contacts.length === 0 && !isLoading ? (
          ...
          <div className="space-y-2">
            {contacts.map((contact) => {
              const canImport = contact.canImport !== false;
              return (
              <div
                key={contact.googleId}
                onClick={() => canImport && toggleContactSelection(contact.googleId)}
                className={`p-3 rounded-lg transition-colors ${canImport ? 'cursor-pointer' : 'cursor-not-allowed opacity-60'}`}
                style={{
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm truncate" style={{ color: 'var(--color-text-secondary)' }}>
                      {contact.email || contact.phone || (canImport ? '' : '(no email - cannot import)')}
                    </p>
                  </div>

                  <div className="flex-shrink-0">
                    {canImport && selectedContacts.has(contact.googleId) ? (
